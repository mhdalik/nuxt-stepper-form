<template>
  ...
  >
              ...
              <tr v-for="candidate in item.candidates">
                ...
                <td>
                  ...
                  <VBtn
                    ...
                    text="Remove"
                    color="red"
                    @click="openDeleteDialog(item.id, candidate.id)"
                  />
                </td>
              </tr>
            </tbody>
    ...
    <template v-slot:item.action="{ item }">
      <VBtn
        v-if="formFillable && item.max_candidates > item.candidates.length"
        text="Add Candidate"
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { title: "For", key: "for", sortable: false },
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      this.loading = true;
      apiPost(`/clubs/assignCandidate`, this.itemObj).then((res) => {
    ...
    async handleDelete() {
      this.loading = true;
      apiPost(`/clubs/detachCandidate`, this.itemObj).then((res) => {
        this.resetAndCloseDialogs();
        this.loadItems();
    ...
    loadItems() {
      apiGet("/clubs/competitions").then((res) => {
